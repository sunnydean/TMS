//forum.unity3d.com/threads/unityengine-mesh-gettrianglestrip-int-is-obsolete.159968/
using UnityEngine;
		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh)
			{
				...
				if (generateStrips)
				{
					...
					int curStripCount = combine.mesh.GetTriangles(combine.subMeshIndex).Length; //FIX 23ap15
					if (curStripCount != 0)
					{
						if( stripCount != 0 )
						{
		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh)
			{
				if (generateStrips)
				{
					...
					int[] inputstrip = combine.mesh.GetTriangles(combine.subMeshIndex); //FIX 23ap15
					if (stripOffset != 0)
					{
						if ((stripOffset & 1) == 1)
						{
...
//			mesh.SetTriangleStrip(strip, 0);
			mesh.SetTriangles(strip, 0); //FIX 23ap15
		else
